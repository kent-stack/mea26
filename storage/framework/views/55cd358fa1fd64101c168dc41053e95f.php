...
<div class="auth-page-shell relative min-h-screen overflow-hidden bg-[#f8fbff]">
    ...
    <div class="relative z-10 flex min-h-screen items-center justify-center px-4 py-8 sm:px-6">
        <div class="w-full max-w-[430px] rounded-[28px] border border-slate-200/80 bg-white/70 p-7 shadow-[0_24px_60px_rgba(15,23,42,0.08)] backdrop-blur-sm sm:p-8">
            ...
            <form class="mt-8 space-y-5" action="<?php echo e(route('login.perform')); ?>" method="POST">
                <?php echo csrf_field(); ?>

                <div>
                    <label for="identifier" class="mb-2 block text-sm font-semibold text-slate-700">ID</label>
                    <input id="identifier" name="identifier" type="text" autocomplete="username" required value="<?php echo e(old('identifier')); ?>" class="block w-full rounded-xl border border-slate-200 bg-slate-100/90 px-4 py-3.5 text-base text-slate-800 placeholder:text-slate-400 focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-100" placeholder="Student ID">
                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\HP\Documents\meapro\mea\resources\views/auth/login.blade.php ENDPATH**/ ?>