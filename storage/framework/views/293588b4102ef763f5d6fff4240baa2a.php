<nav class="navbar-hero hidden md:block z-50 bg-transparent -mt-1 <?php if(in_array(Route::currentRouteName(), ['login', 'register', 'login.perform', 'forgot-password', 'reset-password'])): ?> hidden <?php endif; ?>">
        ...
        <div class="flex items-center justify-center">
            <div class="nav-pill nav-desktop-bar flex w-full items-center justify-between bg-white px-6 py-2 shadow-[0_12px_24px_rgba(30,76,180,0.18)]">
                <a href="/" class="nav-desktop-brand nav-desktop-brand-left flex items-center" aria-label="Danantara Indonesia">
                    <img src="<?php echo e(asset('images/logo_danantara (2).svg')); ?>" alt="Danantara Indonesia" class="h-9 w-auto object-contain">
                </a>
                    ...
                    <div class="nav-pill-user relative">
                        ...
                        <div id="user-menu-dropdown" class="absolute -right-20 top-12 hidden min-w-[210px] rounded-xl border border-slate-200 bg-white p-2 shadow-xl text-slate-800">
                            <div class="border-b border-slate-100 pb-2 text-center text-sm font-semibold text-slate-800">
                                Hi, <?php echo e(auth()->user()->is_admin ? 'Administrator' : (auth()->user()->username ?: auth()->user()->name)); ?>

                            </div>
                            <div class="mt-2">
                                <?php if(auth()->user()->is_admin): ?>
                                    <a href="<?php echo e(route('admin.reports.index')); ?>" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition">Reports</a>
                                    <a href="<?php echo e(route('admin.participants.index')); ?>" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition">Participants</a>
                                    <a href="<?php echo e(url('/module')); ?>" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition">Module</a>
                                <?php endif; ?>
                                <?php if (! (auth()->user()->is_admin)): ?>
                                    <a href="<?php echo e(route('profile')); ?>" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition">Profile</a>
                                <?php endif; ?>
                                <a href="<?php echo e(route('submitted.reports')); ?>" class="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition"><?php echo e(auth()->user()->is_admin ? 'Submitted Announcement' : 'Submitted Reports'); ?></a>
                ...
                <a href="/" class="nav-desktop-brand nav-desktop-brand-right flex items-center" aria-label="PNM">
                    <img src="<?php echo e(asset('images/logo_pnm.svg')); ?>" alt="PNM" class="h-9 w-auto object-contain">
                </a>
            </div>
        </div>
    </div>
...
<!-- Mobile Header -->
<header class="header-container md:hidden fixed inset-x-0 top-0 z-[60] bg-[#f3f4f6]/80 px-4 pt-4 pb-3 shadow-sm backdrop-blur-[1px] <?php if(in_array(Route::currentRouteName(), ['login', 'register', 'login.perform', 'forgot-password', 'reset-password'])): ?> hidden <?php endif; ?>">
    <nav class="main-navigation flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            <img src="<?php echo e(asset('images/logo_danantara (2).svg')); ?>" alt="Danantara Indonesia" class="h-8 w-auto object-contain">
            <img src="<?php echo e(asset('images/logo_pnm.svg')); ?>" alt="PNM" class="h-8 w-auto object-contain">
        </div>
...
<div id="mobile-menu" class="mobile-menu md:hidden fixed inset-0 z-50 hidden <?php if(in_array(Route::currentRouteName(), ['login', 'register', 'login.perform', 'forgot-password', 'reset-password'])): ?> hidden <?php endif; ?>" aria-hidden="true">
                ...
                <?php if(auth()->guard()->check()): ?>
                    <div class="pt-1">
                        ...
                        <div id="mobile-more-menu" class="mobile-more-menu hidden px-2 pb-2">
                            <?php if(auth()->user()->is_admin): ?>
                                <a href="<?php echo e(route('admin.reports.index')); ?>" class="admin-mobile-link block px-4 py-3 text-base font-semibold text-slate-700 transition hover:bg-slate-100">MANAGE REPORTS</a>
                                <a href="<?php echo e(route('admin.participants.index')); ?>" class="admin-mobile-link block px-4 py-3 text-base font-semibold text-slate-700 transition hover:bg-slate-100">PARTICIPANTS</a>
                            <?php endif; ?>
                            <?php if (! (auth()->user()->is_admin)): ?>
                                <a href="<?php echo e(route('profile')); ?>" class="admin-mobile-link block px-4 py-3 text-base font-semibold text-slate-700 transition hover:bg-slate-100">PROFILE</a>
                            <?php endif; ?>
                            <a href="<?php echo e(route('submitted.reports')); ?>" class="admin-mobile-link block px-4 py-3 text-base font-semibold text-slate-700 transition hover:bg-slate-100"><?php echo e(auth()->user()->is_admin ? 'SUBMITTED ANNOUNCEMENT' : 'SUBMITTED REPORTS'); ?></a>
                            <form method="POST" action="<?php echo e(route('logout')); ?>" class="pt-1 text-center">
                                <?php echo csrf_field(); ?>
...
<?php /**PATH C:\Users\HP\Documents\meapro\mea\resources\views/components/navbar.blade.php ENDPATH**/ ?>